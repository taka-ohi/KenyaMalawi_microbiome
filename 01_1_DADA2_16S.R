...
### load database (SILVA; for 16S rRNA)
# obtained from here https://zenodo.org/records/1172783 
DATABASE <- "~/Desktop/analysis/Database/silva_nr_v132_train_set.fa.gz" # set your SILVA directory


### performing DADA2
# set working directory and list files
setwd("Data/2019kenya_malawi_16S/")  ## CHANGE ME to the directory containing the fastq files.
filez <- list.files()

# prepare directory for output files
dir.create("../../01_DADA2_out")
...
ggsave(plot=p, file="../../01_DADA2_out/errors_16S.png") # did not work for some reason but can be skipped.
...
head(track)
write.table(track,file="../../01_DADA2_out/track_16S.txt", quote=F)
...
write.table(taxa, file="../../01_DADA2_out/taxonomy_16S.txt", quote=F)
write.table(seqtab.nochim, file="../../01_DADA2_out/seqtabnochim_16S.txt", quote=F)
...
taxa_names(ps) <- paste0("Prok_ASV", seq(ntaxa(ps)))
ps

otu_table.t<-t(ps@otu_table)
ps.t<-cbind(otu_table.t,ps@tax_table)
write.table(ps.t,  file="../../01_DADA2_out/ASV_table_16S.txt", quote=F, sep = "\t")
...
rared_b_ASV.t <- rrarefy(b_ASV.t, min(rowSums(b_ASV.t)))

sample_names <- row.names(rared_b_ASV.t)
sample_names <- sub("_16S", "", sample_names) # just removed "_16S" from the sample name
row.names(rared_b_ASV.t) <- sample_names

rared_b_ASV <- t(rared_b_ASV.t)
rared_b_taxa <- ps.t_wo_others[,(ncol(ps.t_wo_others)-6):(ncol(ps.t_wo_others))] # get taxa info

# remove rows that contains only 0 counts after rarefaction
rared_b_ASV_wo_0 <- rared_b_ASV[apply(rared_b_ASV, 1, function(row) !all(row == 0)), ]
rared_b_ASV_wo_0 <- as.data.frame(rared_b_ASV_wo_0)

# combine the count data and taxa data
rared_b_ASV_wo_0 <- rared_b_ASV_wo_0 |> rownames_to_column(var = "ASV")
rared_b_taxa <- rared_b_taxa |> rownames_to_column(var = "ASV")

rared_b_ASV.table <- merge(rared_b_ASV_wo_0, rared_b_taxa, by = "ASV", all.x = T, all.y = F, sort = F) # combine
